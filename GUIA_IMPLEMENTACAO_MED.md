# 🔄 Guia de Implementação - Sistema MED

## 📋 O que é MED?

**MED** = Mecanismo Especial de Devolução

Sistema que permite clientes solicitarem devolução de valores em casos específicos (erro na transação, cobrança indevida, etc.).

## 🚀 Arquivos Criados:

1. ✅ `SQL_CRIAR_TABELA_MED.sql` - SQL para criar tabela
2. ✅ `MED_REQUESTS_PAGE_CODE.txt` - Página do cliente
...
```sql
-- Execute TODO o conteúdo de: SQL_CRIAR_TABELA_MED.sql
```
...
### 2. Criar Página do Cliente

1. Crie: `src/pages/MEDRequests.tsx`
2. Copie o código de: `MED_REQUESTS_PAGE_CODE.txt`
3. Salve
...
// Nas rotas protegidas, adicione:
<Route path="/med" element={<ProtectedRoute><MEDRequests /></ProtectedRoute>} />
```

### 4. Adicionar no Menu do Cliente

Edite `src/components/Layout.tsx`:

```typescript
import { RefreshCw } from 'lucide-react' // ou outro ícone
...
### 5. Criar Página Admin MED

Crie `src/pages/AdminMED.tsx` com:
...
export function AdminMED() {
  ...
  const loadRequests = async () => {
    try {
      ...
      toast.error('Erro ao carregar solicitações MED')
    } finally {
      setLoading(false)
    }
  }
...
// Nas rotas admin:
<Route path="/admin/med" element={<ProtectedRoute><AdminMED /></ProtectedRoute>} />
```

### 7. Adicionar no Menu Admin

Edite `src/components/Layout.tsx` (menu admin):
...
### Admin:
1. Acessa `/admin/med`
2. Vê todas as solicitações
3. Pode:
   - **Aprovar** - Adiciona observações
   - **Rejeitar** - Informa motivo
   - **Concluir** - Marca como processado

### Status:
- **Pendente** - Aguardando análise
- **Aprovado** - Admin aprovou, aguarda processamento
- **Rejeitado** - Admin rejeitou
- **Concluído** - Devolução processada

## ✅ Checklist:

- [ ] Executar SQL
- [ ] Criar MEDRequests.tsx
- [ ] Criar AdminMED.tsx
- [ ] Adicionar rotas
- [ ] Adicionar nos menus
- [ ] Testar fluxo completo

Pronto! Sistema MED completo! 🚀
